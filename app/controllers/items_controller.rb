class ItemsController < ApplicationController
  ...
  def index
    ...
    @items = @q.result(:distinct => true).includes(:user, :recommendations, :transactions, :category).page(params[:page]).per(10)
    @location_hash = Gmaps4rails.build_markers(@items.where.not(:location_latitude => nil)) do |item, marker|
      ...
      marker.infowindow "<h5><a href='/items/#{item.id}'>#{item.id}</a></h5><small>#{item.location_formatted_address}</small>"
    end
  end
  ...
  def current_user_must_be_item_user
    ...
    def item_params
      params.require(:item).permit(:name, :price, :description, :seller_id, :image, :location, :category_id, :available)
    end
end
